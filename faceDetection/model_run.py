...
def InsertLogo_2(image1, image2, x, y):
	...
	for i in range(rows):
		for j in range(cols):
			if mask[i][j] != 0:
				if i+x >= 720 or j+y >= 720:
					#img1[1279][719]=img2[i][j]	
					#print('no')
					#print(i+x)
					#print(j+y)
					one = 'no'
...
def show_results(img,results, img_width, img_height, model_age, model_gender, model_emotion):
	...
	for i in range(len(results)):
		...
		y_avg=(ymin2+ymax2)/2
		x_avg=(xmin2+xmax2)/2
		result = transparentOverlay(img_cp,dog_img,(xmin2+20,ymin2),1)
		#InsertLogo_2(img_cp, dog_img, xmin2, ymax2-ymin2)
